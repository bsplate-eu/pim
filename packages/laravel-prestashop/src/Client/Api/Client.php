...
class Client
{
    ...
    const PS_COMPATIBLE_VERSION_MAX = '9.99.99.99';
    ...
    protected function checkRequest($request)
    {
        ...
        if (!isset($details->errors->error)) {
            throw new PrestashopApiException($message, $httpStatus);
        } else {
            ...
            throw new PrestashopApiException($message, $prestashopCode);
        }
    }
    ...
    public function isPrestashopVersionSupported($version)
    {
        if (version_compare($version, self::PS_COMPATIBLE_VERSION_MIN, '>=') === false ||
            ...
            throw new PrestashopApiException('PrestaShop error. Incompatible version (min:%s, max:%s).');
        }
    }
    ...
    protected function executeRequest($url, $curl_params = [])
    {
        ...
        if ($status_code === 0 || $error) {
            ...
            throw new PrestashopApiException('Error request to the PrestaShop API. ' . $error,  $httpStatus);
        }
        $index = $info['header_size'];
        if ($index === false && $curl_params[CURLOPT_CUSTOMREQUEST] !== 'HEAD') {
            throw new PrestashopApiException('Error request to the PrestaShop API. Bad HTTP response.',  400);
        }
    ...
    public function add($options)
    {
        return $this->withAttempts(function () use (&$options) {
            ...
            if (isset($options['resource'], $options['postXml']) || isset($options['url'], $options['postXml'])) {
                ...
                throw new PrestashopApiException('Error PrestaShop. Bad parameters given.',  400);
            }
