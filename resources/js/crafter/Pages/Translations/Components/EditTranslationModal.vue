<template>
  <Modal>
    ...
    <template #title> {{ $t("crafter", "Edit translation") }}</template>
    <template #content>
...
import { LanguageLine } from "crafter/types/models";
import { Button, Modal, TextInput, IconButton } from "crafter/Components";
...
const submit = (setIsOpen: Function) => {
  form.post(route("crafter.translations.update", [props.languageLine.id]), {
    onFinish: () => {
      setIsOpen();
      toast.success(
        trans("crafter", "Translation was successfully updated")
      );
    },
  });
};
