  ...
  <form>
    <div class="my-4">
      <label class="mb-1 block text-sm font-medium text-gray-700" for="file">
        ...
        <template v-else> {{ $t("crafter", "Choose file") }} </template>
      </label>
      <input
        id="file"
        ref="file"
    ...
    <Multiselect
      ...
      :options="locales"
      :label="$t('crafter', 'Languages to import')"
      mode="single"
      name=""
    />
    <Checkbox
      v-model="onlyMissing"
      class="mt-2"
      :label="$t('crafter', 'Do not override existing translations')"
    />
  </form>
</template>

<script lang="ts" setup>
import { Multiselect } from "crafter/Components";
import { ref, watchEffect } from "vue";
import Checkbox from "crafter/Components/Checkbox.vue";
