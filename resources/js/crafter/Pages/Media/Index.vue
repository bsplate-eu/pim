<template>
  <PageHeader :title="$t('crafter', 'Media')">
    <ImageUploadModal @imageUploaded="imageUploaded">
      <template #button="{ setIsOpen }">
        <Button
          v-can="'crafter.media.upload'"
          :left-icon="PlusIcon"
          as="button"
          @click="() => setIsOpen(true)"
        >
          {{ $t("crafter", "Upload media") }}
        </Button>
      </template>
    </ImageUploadModal>
  </PageHeader>
    ...
    <Card :no-padding="selectedView === 'list'">
      <template #tabs>
        <Tab
          ...
          :selected="routeIsActive(route('crafter.media.index'))"
        >
          {{ $t("crafter", "All") }}
        </Tab>
        <Tab
          :href="route('crafter.media.images')"
          :selected="routeIsActive(route('crafter.media.images'))"
        >
          {{ $t("crafter", "Images") }}
        </Tab>
        <Tab
          :href="route('crafter.media.files')"
          :selected="routeIsActive(route('crafter.media.files'))"
        >
          {{ $t("crafter", "Files") }}
        </Tab>
      </template>
            ...
            <li v-for="file in data.data" :key="file.id" class="relative">
              ...
              >
                ...
                >
                  <span class="sr-only">
                    {{
                      $t("crafter", "View details for :name", {
...
import { formatBytes } from "crafter/helpers";
...
const baseUrl = computed(() => {
  if (routeIsActive(route("crafter.media.images"))) {
    return route("crafter.media.images");
  }

  if (routeIsActive(route("crafter.media.files"))) {
    return route("crafter.media.files");
  }

  return route("crafter.media.index");
});
