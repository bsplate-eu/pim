<template>
  <div class="w-full">
    ...
    <div v-auto-animate class="mt-1 rounded-md shadow-sm sm:col-span-2 sm:mt-0">
        <div v-if="filteredFiles.length" class="mb-3 flex justify-end">
            <Button size="sm" color="gray" variant="outline" @click="downloadAll">
                Pobierz Wszystkie
            </Button>
        </div>
      <div
      ...
      >
        ...
        >
          ...
          >
            ...
            <button
              type="button"
              class="text-danger-500 hover:text-danger-700 absolute -top-2 -right-2 rounded-full bg-white"
          ...
          >
            ...
            <div class="text-sm text-gray-500">
              ...
              <p>{{ formatBytes(file.size) }}</p>
                ...
                <div class="flex flex-col p-2 gap-1">
                    <Button as="a" :href="file.original_url" size="sm" color="gray" variant="outline" :download="file.file_name">Pobierz</Button>
                    <Button size="sm" color="gray" variant="outline" @click.stop.prevent="copyUrl(file)">
                        Kopiuj adres
                    </Button>
                </div>
            </div>
            ...
            <div class="col-span-2 space-y-3">
              ...
              >
                ...
                />
                <TextInput
                  v-model="file.order_column"
                  size="sm"
                  label="Order"
                  :name="`${file.uuid ?? file.id}_order`"
                />
              </slot>
            </div>
            <button
              type="button"
              class="absolute top-2 right-3 rounded-full text-gray-500 hover:text-gray-700"
              @click="removeImage(file)"
            >
            ...
            <div class="space-y-1">
              <div class="flex text-sm text-gray-600">
                ...
                <span v-if="maxNumberOfFiles && maxNumberOfFiles > 1">
                  >&nbsp;maximum of {{ maxNumberOfFiles }} files</span
                >
...
import { formatBytes, getFileExtension } from "crafter/helpers";
...
const copyUrl = async (file: UploadedFile) => {
    await navigator.clipboard.writeText(file.original_url);
    toast.success(trans("crafter", "Copied image URL"));
};

const downloadAll = async () => {
    const ids = filteredFiles.value.map(f => f.id);
    const res = await axios.post('/admin/media-zip', { ids }, { responseType: 'blob' });
...
const filteredFiles = computed(() =>
  files.value
    ? files.value?.filter((file) => !file.action || file.action !== "delete").sort(
...
const uploadFiles = (filesToUpload: File[]) => {
  ...
  filesToUpload.forEach((file: File) => {
    if (file.size > props.maxFileSize!) {
      toast.error(trans("crafter", "File size is too big!"));
      return;
    }
