...
import { Pagination } from "crafter/types/pagination";
import { usePaginationLinks } from "crafter/hooks/usePaginationLinks";
...
watch(
  ...
  () => {
    form.get(
      route(route().current(), { ...route().params, per_page: form.per_page })
    );
  }
);
</script>
