<template>
  <div
    class="avatar relative flex flex-shrink-0 items-center justify-center overflow-hidden bg-gray-100"
    :class="styles"
  >
    <img v-if="src" :src="src" class="absolute inset-0 h-full w-full" alt="" />
    <span
      v-else-if="name"
      class="absolute inset-0 flex h-full w-full items-center justify-center font-medium uppercase leading-none text-primary-800"
    >
      {{ initials }}
    </span>
    <UserIcon v-else class="h-9 self-end text-gray-400" />
  </div>
</template>
