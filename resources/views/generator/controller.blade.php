...
use {{ $modelNamespace }}\{{ $relation['model'] }};
@endif
@endforeach
...
@endif
        return redirect()->route('{{$modelRoute}}.index')->with(['message' => ___('crafter', 'Operation successful')]);
    }
...
@endif
        if (session('{{$modelNamePluralLowerCase}}_url')) {
            return redirect(session('{{$modelNamePluralLowerCase}}_url'))->with(['message' => ___('crafter', 'Operation successful')]);
        }
