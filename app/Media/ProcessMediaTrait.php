...
use App\Media\Exceptions\FileCannotBeAdded\FileIsTooBig;
use App\Media\Exceptions\FileCannotBeAdded\TooManyFiles;
...
trait ProcessMediaTrait
{
    ...
    public function processMedia(Collection $inputMedia): void
    {
        ...
        $this->getRegisteredMediaCollections()->keyBy('name')->each(function ($mediaCollection) use ($inputMedia) {
        ...
        if (isset($inputMedium['id']) && $inputMedium['id']) {
            if ($medium = app(Media::class)->find($inputMedium['id'])) {
                if (isset($inputMedium['action']) && $inputMedium['action'] === 'delete') {
                    ...
                    $medium->custom_properties = $inputMedium['custom_properties'];
                    $medium->order_column = $inputMedium['order_column'];
                    $medium->save();
                }
            }
            ...
            $this->addMedia($mediumFileFullPath)
                ->withCustomProperties($inputMedium['custom_properties'])
    ...
    public function validate(Collection $inputMediaForMediaCollection, MediaCollection $mediaCollection): void
    {
        ...
        $inputMediaForMediaCollection->each(function ($inputMedium) use ($mediaCollection) {
            if (isset($inputMedium['action']) && $inputMedium['action'] === 'add') {
                $mediumFileFullPath = Storage::disk('uploads')->path($inputMedium['path']);
    ...
    public function validateCollectionMediaCount(
        ...
        if ($mediaCollection->collectionSizeLimit && ! $mediaCollection->singleFile) {
            ...
            $forAddMediaCount = $inputMediaForMediaCollection->filter(static function ($medium) {
                return isset($medium['action']) && $medium['action'] === 'add';
            })->count();
            $forDeleteMediaCount = $inputMediaForMediaCollection->filter(static function ($medium) {
                return isset($medium['action']) && $medium['action'] === 'delete' ? 1 : 0;
            })->count();
            $afterUploadCount = ($forAddMediaCount + $alreadyUploadedMediaCount - $forDeleteMediaCount);

            if ($afterUploadCount > $mediaCollection->collectionSizeLimit) {
                throw TooManyFiles::create($mediaCollection->collectionSizeLimit, $mediaCollection->getName());
            }
        }
    }
    ...
    {
        if ($mediaCollection->acceptsMimeTypes) {
            $this->guardAgainstInvalidMimeType($mediumFileFullPath, $mediaCollection->acceptsMimeTypes);
        }
    }
