    ...
    private function flushProductBuffer(): void
    {
        ...
        try {
            ...
            foreach ($results as $index => $result) {
                ...
                if (($result['status'] ?? '') === 'ok') {
                    // OpenCart connector zwraca product_id na top-level wyniku; Presta/LiteCart pod 'data'.
                    $externalId = $result['product_id'] ?? $result['id_product'] ?? $result['data']['product_id'] ?? $result['data']['id_product'] ?? $result['data']['id'] ?? null;
