...
class InstallCommand extends Command
{
    ...
    public $signature = 'crafter:install      {--overwrite        : Overwrite files}
    ...
    public $description = 'Install Crafter';

    protected $assetsNamespace = 'crafter';

    public function handle(): int
    {
        $this->components->info("Crafter installation started");

        if ($this->call("crafter:test-db-connection") === 0) {
            $this->components->error("In order to install Crafter, please check and configure the database connection");
        ...
        $this->call('vendor:publish', [
            '--provider' => "Crafter\CrafterServiceProvider",
            '--tag' => "crafter-migrations",
        ]);
        ...
        $this->components->info("Crafter successfully installed");

        if (! $this->option('no-user')) {
            $this->call(
                'crafter:create-admin-user',
                [
        ...
        $this->components->warn('Don\'t forget to execute the "npm install && npm run crafter:dev" command to build newly published assets.');
    ...
    protected function editPackageJson(callable $callback, $dev = true)
    {
        ...
        $this->components->task("Updating package.json", function () use ($callback, $dev) {
            ...
            $packages['scripts'] = $packages['scripts'] + [
                "crafter:dev" => "vite --config crafter.vite.config.js",
                "crafter:build" => "vite build --config crafter.vite.config.js",
            ];
    ...
    {
//        $this->components->task("Publishing Crafter root template", function () {
//            $this->copyIfNotExistOrOverwrite(__DIR__ . '/../../stubs/resources/views/crafter.blade.php', resource_path('views/crafter.blade.php'));
//        });
//
//        $this->components->task("Publishing Crafter assets", function () {
...
//            $this->copyIfNotExistOrOverwrite(__DIR__ . '/../../stubs/crafter.tailwind.config.js', base_path('crafter.tailwind.config.js'));
//            $this->copyIfNotExistOrOverwrite(__DIR__ . '/../../stubs/crafter.vite.config.js', base_path('crafter.vite.config.js'));
//
//            if ($generateTSConfig) {
...
//        });
    ...
    {
        if (! Route::has('crafter.home')) {
            $this->components->task("Registering Crafter routes");
            file_put_contents(base_path('routes/web.php'), PHP_EOL . "\n\nRoute::crafter('admin');", FILE_APPEND);
        } else {
            $this->printSkipped("Crafter routes are already installed");
        }
    }
    ...
    protected function scanAndPublishTranslations(): void
    {
        ...
        $this->components->task("Generate translations for permissions", function () {
            $this->call('crafter:generate-permission-translations');
            $this->call('crafter:generate-locale-translations');
        });

        $this->components->task("Scan codebase used for translations", function () {
            $this->call('crafter:scan-translations');
        });

        $this->components->task("Publish translations (generate JSON files)", function () {
            $this->call('crafter:publish-translations');
        });
    }
    ...
    protected function installAdvancedLoggerPackage()
    {
        if (! $this->option('without-logger')) {
            $this->newLine();
            $this->components->info("Crafter supports the package Request Logger that automatically logs all incoming requests with handy information for debugging problems.");
            ...
            if ($install === 'yes') {
                $this->call('crafter:install-advanced-logger');
            }
        }
    }
