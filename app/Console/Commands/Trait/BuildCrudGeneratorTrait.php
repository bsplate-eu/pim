...
trait BuildCrudGeneratorTrait
{
    ...
    protected function baseBuildReplacements(): array
    {
        return [
            ...
            'modelRoute' => "crafter." . Str::kebab(Str::plural($this->className)),
    ...
    protected function requestBuildReplacements(string $requestClassName, string $request, string $permissionName): array
    {
        ...
        return array_merge($this->baseBuildReplacements(), [
            ...
            'requestNamespace' => $this->requestNamespace,
            'rules' => $rules,
            'permissionName' => "crafter." . Str::kebab($this->className) . ".$permissionName",
        ]);
    }
    ...
    {
        $baseRouteName = "crafter." . Str::kebab(Str::plural($this->className));
    ...
    protected function buildPermissionMigrationReplace(): array
    {
        return [
            ...
            collect(['index', 'create', 'edit', 'destroy'])->map(function ($permission) {
                return "crafter." . Str::kebab($this->className) . ".$permission";
            }),
            'permissionGuardName' => 'crafter',
        ];
    }
    ...
    protected function buildVueIndexColumns(): mixed
    {
        return $this->indexColumns->map(function ($column) {
            ...
            if ($column['publishable']) {
                ...
                $dataCellContent =
                    '<Publish :publishedAt="item.' . $column["name"] . '" :updateUrl="route(\'crafter.' . $updateRoute . '\', item.id)" columnName="' . $column["name"] . '" mode="' . $datePickerMode . '"/>';
            }
    ...
    protected function getEditVueImports(): string
    {
        ...
        if (! empty($this->mediaCollections)) {
            $return .= 'import {getMediaCollection} from "crafter/helpers";';
        }
    ...
    if(url.length > 1) {
      window.location = route(\'crafter.' . Str::kebab(Str::plural($this->className)) . '.export\', url.pop()).slice(0, -1);
    } else {
      window.location = route(\'crafter.' . Str::kebab(Str::plural($this->className)) . '.export\');
    }
}';
        }
...
const { availableLocales, currentLocale, translatableDefaultValue, getLabelWithLocale } = useFormLocale();
    ...
    protected function getTypeScriptImports(): string
    {
        $return = "";
        if (! empty($this->mediaCollections)) {
            $return .= 'import type { UploadedFile } from "crafter/types";';
        }
    ...
    protected function mapRelationsData(array $columns): Collection
    {
        return collect($columns)->map(function ($column) {
            ...
            if ($type === 'belongsToMany') {
                ...
                $tableName = isset($parts[1]) ? $parts[1] : Str::lower(Arr::join(Arr::sort([$this->className, $relatedModel]), '_'));
            } else {
    ...
    protected function buildModelRelations(): string
    {
        ...
        return $this->relations?->map(function ($relation) {
            ...
            if ($relation['type'] === 'belongsToMany') {
                $relationDefinition = "{$relation['model']}::class{$tableName}{$foreignKey}{$ownerKey}";
            } else {
                $relationDefinition = "{$relation['model']}::class{$foreignKey}{$ownerKey}";
            }
