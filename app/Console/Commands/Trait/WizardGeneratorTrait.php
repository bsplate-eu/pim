    ...
    protected function getAndSetRelations()
    {
        if ($this->shouldAskForOptionValue('add-relation')) {
            ...
            if ($this->components->choice(
                ...
                $allModels = File::allFiles(app_path('Models'));

                foreach ($allModels as $modelFile) {
                    $models[] = $modelFile->getFilenameWithoutExtension();
                }

                $models[] = $this->className;
                ...
                if ($relation['type'] === 'belongsToMany') {
                    ...
                    $relation['foreignKey'] = $this->components->choice(
                        ...
                        default: $intColumns->first(fn ($column) => $column === Str::lower("{$relation['model']}_id")) ?? $intColumns->first(),
                    );
                }
