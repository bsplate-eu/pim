...
trait CrudGeneratorTrait
{
    ...
    protected string $controllerNamespace = 'App\Http\Controllers\Admin';
    ...
    protected string $exportNamespace = 'App\Exports\Admin';
    ...
    protected function setTableAndClassName(string $tableName)
    {
        ...
        $this->indexColumns = $this->tableColumns->where('hidden', false);
    ...
    protected function checkWithExistsModel()
    {
        ...
        if ($this->files->exists($modelPath)) {
            ...
            if (strpos($file, "/** Auto-generated by Crafter */") === false) {
    ...
    {
        $this->requestNamespace = "App\Http\Requests\Admin\\" . $this->className;
    ...
    protected function buildModel()
    {
        if ($this->overwriteModel) {
            $this->components->task('Model', function () {
                ...
                $template = view('generator.model', $replace)->render();
    ...
    protected function buildController()
    {
        ...
        $this->components->task($this->controllerName, function () {
            ...
            $template = view("generator.controller", $replace)->render();
    ...
    protected function buildRequests(string $name, string $permissionName = null)
    {
        ...
        $this->components->task($requestName, function () use ($name, $permissionName, $requestName) {
            ...
            $this->files->put($requestPath, view('generator.request_rule', $replace)->render());
        });
    ...
    protected function buildRoutes()
    {
        $this->components->task("Routes", function () {
            ...
            $content = view('generator.route', $replace)->render();

            $this->appendIfNotAlreadyAppended(
                base_path('routes/crafter.php'),
    ...
    {
        $typeScriptPath = $this->makeDirectory(resource_path("js/crafter/Pages/{$this->className}/types.d.ts"));
    ...
    protected function buildPermissionsMigration()
    {
        $this->components->task("Migration", function () {
            ...
            $template = view("generator.permission_migration", $replace)->render();
    ...
    protected function buildExport()
    {
        if ($this->withExport) {
            $this->components->task("Export", function () {
                ...
                $template = view("generator.export", $replace)->render();
    ...
    protected function buildDryRun()
    {
        ...
        $this->info(
            "php artisan crafter:generate-crud " . $this->tableName .
    ...
    protected function appendLinkInSidebar()
    {
        $this->components->task("Sidebar", function () {
            $permission = "crafter." . Str::kebab($this->className) . ".index";
            $link = Str::kebab(Str::plural($this->className));
            $linkName = Str::headline(Str::plural($this->className));

            $sidebar = resource_path("js/crafter/Components/Sidebar.vue");
            $file = $this->files->get($sidebar);
            $append = '<SidebarItem
                        :href="route(\'crafter.' . $link . '.index\')"
